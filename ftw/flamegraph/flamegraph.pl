...
my $egghash = 0;                # color by egg name
my $palette = 0;                # if we use consistent palettes (default off)
my %palette_map;                # palette map hash
my %egg_map;                    # palette map hash
...
USAGE: $0 [options] infile > outfile.svg\n
	...
	--egghash     # colors are keyed by python egg hash
...
sub egghash {
	my $name = shift;
	if (exists $egg_map{$name}) {
		return $egg_map{$name};
	} else {
    $egg_map{$name} = hex(substr(md5_hex($name), 0, 8)) / 1000000000;
		return $egg_map{$name};
	}
}

sub color {
	my ($type, $hash, $egghash, $name) = @_;
	my ($v1, $v2, $v3);
    ...
    $v1 = egghash($eggname);
    $v2 = egghash(scalar reverse $eggname);
    $v3 = egghash(sort $eggname);
  }
  else {
		$v1 = rand(1);
		$v2 = rand(1);
	...
	if (defined $type and $type eq "rainbow") {
		my $r = 100 + int(1000 * $v3 % 130);
		my $g = 70 + int(1000 * $v1 % 150);
		my $b = 70 + int(1000 * $v2 % 150);
		return "rgb($r,$g,$b)";
	}
...
sub color_map {
	...
	if (exists $palette_map{$func}) {
		...
		$palette_map{$func} = color($colors, $hash, $egghash, $func);
		return $palette_map{$func};
	}
}
...
while (my ($id, $node) = each %Node) {
	...
	if ($func eq "-") {
		...
		$color = color($colors, $hash, $egghash, $func);
	}
